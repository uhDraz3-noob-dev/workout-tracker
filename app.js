// Simple, hashed (not plaintext) PIN gate for static sites.
// Stores SHA-256(pin) in localStorage. Do not use for high-security needs.

const PIN_KEYS = {
  hash: "wt.pinHash",
  locked: "wt.locked" // "1" = require PIN now
};
...
async function ensurePinGate() {
  const hasPin = !!localStorage.getItem(PIN_KEYS.hash);
  const shouldLock = localStorage.getItem(PIN_KEYS.locked) === "1";
...
function showOverlay(show) {
  $("pin-backdrop").style.display = show ? "block" : "none";
  $("pin-modal").style.display = show ? "flex" : "none";
}

function hideOverlay() { showOverlay(false); }

// SETUP FLOW (no PIN yet)
function showPinSetup() {
  showOverlay(true);
  $("pin-title").textContent = "Set a 4–8 digit PIN";
  $("pin-sub").textContent = "You'll use this to unlock the tracker.";
  $("pin-confirm").style.display = "";
  $("pin-input").value = "";
  $("pin-confirm").value = "";
  ...
  form.onsubmit = async (e) => {
    e.preventDefault();
    const p1 = $("pin-input").value.trim();
    const p2 = $("pin-confirm").value.trim();

    if (!/^\d{4,8}$/.test(p1)) {
      $("pin-error").textContent = "PIN must be 4–8 digits.";
      return;
    }
    if (p1 !== p2) {
      $("pin-error").textContent = "PINs do not match.";
      return;
    }
    const hash = await sha256Hex(p1);
    localStorage.setItem(PIN_KEYS.hash, hash);
    localStorage.removeItem(PIN_KEYS.locked); // unlocked on create
    hideOverlay();
  };
}

// VERIFY FLOW (PIN exists or user re-locked)
function showPinVerify() {
  showOverlay(true);
  $("pin-title").textContent = "Enter PIN to unlock";
  $("pin-sub").textContent = "";
  $("pin-confirm").style.display = "none";
  $("pin-input").value = "";
  $("pin-error").textContent = "";
  ...
  form.onsubmit = async (e) => {
    ...
    if (!/^\d{4,8}$/.test(pin)) {
      $("pin-error").textContent = "Invalid PIN format.";
      return;
    }
    const given = await sha256Hex(pin);
    const saved = localStorage.getItem(PIN_KEYS.hash);
    if (given === saved) {
      localStorage.removeItem(PIN_KEYS.locked); // unlocked
      hideOverlay();
    } else {
...
// Public helpers you can call from your UI:
window.lockNow = function lockNow() {
  localStorage.setItem(PIN_KEYS.locked, "1");
  showPinVerify();
};

window.clearPin = function clearPin() {
  localStorage.removeItem(PIN_KEYS.hash);
  localStorage.setItem(PIN_KEYS.locked, "1"); // force setup next time
  showPinSetup();
};
...
document.addEventListener("DOMContentLoaded", () => {
  if (!window.crypto || !crypto.subtle) {
    console.error("Web Crypto not available; PIN disabled.");
    return;
  }
  ensurePinGate();
});
